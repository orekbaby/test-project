  ...
  const onSaveNotice = async (data: NoticeFormProps) => {
    try {
      ...
      const noticeData = {
        ...
        status: data.status || 1, // Default to draft status if not specified
        authorId: 1 // You need to get this from user context/auth
      };

      const result = await addNotice(noticeData).unwrap();
      toast.info(result.message);
      navigate('/app/notices');
    } catch (error) {
      const errorMessage = getErrorMsg(error as FetchBaseQueryError | SerializedError);
